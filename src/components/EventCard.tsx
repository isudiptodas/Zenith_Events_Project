...
function EventCard({ id, name, desc, location, date, poster, points, link, expired, setVisible }: eventProps) {
  return (
    <>
      <motion.div initial={{ opacity: 0, filter: "blur(20px)" }} viewport={{once:true}} whileInView={{ opacity: 1, filter: "blur(0px)" }} transition={{ duration: 0.3, delay: 0.2, ease: "easeInOut" }} className={`w-full group cursor-pointer h-auto flex flex-col justify-start items-center rounded-2xl ${expired ? "bg-linear-to-br from-white via-gray-200 to-gray-400" : "bg-linear-to-br from-blue-300 via-blue-500 to-blue-800"} pt-1 px-1 pb-8 relative overflow-hidden`}>
        ...
        <div className={`w-full z-20 bg-zinc-950 rounded-2xl h-auto flex flex-col justify-start items-center`}>
          ...
          <div className={`w-full flex justify-between items-center gap-3 px-3`}>
            <p onClick={() => window.open(link, '_blank')} className={`w-full ${expired ? "hidden" : "block"} mt-2 py-2 rounded-md bg-white text-black text-center cursor-pointer select-none mb-4 active:opacity-80 duration-200 ease-in-out`}>Register</p>
            <p onClick={setVisible} className={`w-full ${expired ? "hidden" : "block"} ${poster ? "block" : "hidden"} mt-2 py-2 rounded-md bg-white text-black text-center cursor-pointer select-none mb-4 active:opacity-80 duration-200 ease-in-out`}>View Poster</p>
          </div>
